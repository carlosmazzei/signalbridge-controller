...
- **Payload:**
  ...
  - `payload[2]`: column LED bitmask — bit N (LSB-first) drives row N of the selected column; `1` = LED on, `0` = LED off. Bits for LEDs not set are cleared on the selected column only; other columns are untouched.
